...
- **OLTP store** with high concurrency and horizontal scaling; p99 write latency under 5ms and read latency under 50ms under the workload documented in [TCO-Comparisons.md](docs/TCO-Comparisons.md)
...
- **Lower TCO** at scale than OLTP + ETL + warehouse stacks — see the [TCO worksheet](docs/TCO-Comparisons.md)
...
- **OLTP path** — point reads and bounded partition reads through Cassandra's request path. Target: p99 write < 5ms, p99 read < 50ms under documented workload.
- **OLAP path** — wide scans and aggregations via the Spark Bulk Reader, reading SSTable files directly from coordinated snapshots. Does not contend with OLTP.
