...
Proof-of-Concept that takes 4 minutes to demonstrate a  
Best-in-class Hybrid Transactional Analytical Processing (HTAP) Data Platform
...
The rest of the README is broken up into four segments:
...
## Demo Quick Start
• Bring up the whole stack in under 5 minutes, and start ingesting 500 events per second.
...
```sql
podman exec cassandra cqlsh cassandra -e "SELECT * FROM demo.events LIMIT 3;"
```
...
```shell
podman exec -it presto presto-cli --execute "SHOW SCHEMAS FROM cassandra;"

podman exec -it presto presto-cli --execute "SELECT * FROM cassandra.demo.events LIMIT 100;"

podman exec -it presto presto-cli --execute "SELECT event_type, COUNT(*) FROM cassandra.demo.events GROUP BY event_type;"
```
To watch query progress in browser, open http://localhost:8088/ui/  

• Example **SparkSQL Queries**

A simple query using the Cassandra-Spark-Connector
```shell
spark-sql --packages com.datastax.spark:spark-cassandra-connector_2.12:3.5.1 \
  --conf spark.cassandra.connection.host=cassandra \
  -e 'SELECT * FROM cass_v LIMIT 1;'
```

Queries using the Cassandra Spark Bulk Reader (via the Cassandra Sidecar) 
```sql
podman exec -it spark /opt/spark/bin/spark-sql

CREATE OR REPLACE TEMP VIEW events_pt
...
SELECT event_type, COUNT(*) AS cnt, MIN(event_time) AS first_seen, MAX(event_time) AS last_seen
  FROM events_pt GROUP BY event_type ORDER BY cnt DESC LIMIT 10;
```
To watch query progress in browser, open http://localhost:4040/  

The simple query goes through Cassandra's CQL and the jvm, suitable for per-partition reads.  The bulk reader query reads a snapshot of the sstables files directly (off disk), suitable for point-in-time consistency and heavy bulk/analytics-style reads. The Cassandra Sidecar can also bulk write directly to sstables files, see below for import/export from Parquet examples.

• Example **Accord Transactions**  
TODO– provide a simpler accord transaction to begin with


A more complex transaction.
The following provides "exactly-once in-order session timeline projections"

```sql
podman exec -it cassandra bash

# Get
cqlsh cassandra -e "SELECT user_id, event_type FROM demo.events LIMIT 1;"
USER_ID="<from_above>"
EVENT_TYPE="<from_above>"

# Create new values
SESSION_ID=$(cat /proc/sys/kernel/random/uuid)
EVENT0=$(cat /proc/sys/kernel/random/uuid)
EVENT1=$(cat /proc/sys/kernel/random/uuid)

# Do the rest in cqlsh
cqlsh cassandra
-- Create the session
 INSERT INTO demo.sessions_open (user_id, session_id) VALUES ('${USER_ID}', ${SESSION_ID});
 SELECT * FROM demo.sessions_open;

-- First Accord Transaction on seq=0
--  (Apply seq=0 (bootstraps the session timeline))
 BEGIN TRANSACTION
  LET session_ok = (SELECT session_id
                    FROM demo.sessions_open
                    WHERE user_id='${USER_ID}' AND session_id=${SESSION_ID});

  LET already0 = (SELECT seq
                  FROM demo.session_seq_applied
                  WHERE user_id='${USER_ID}' AND session_id=${SESSION_ID} AND seq=0);

  LET event0_exists = (SELECT event_id
                       FROM demo.events
                       WHERE event_id=${EVENT0});

  IF session_ok IS NOT NULL AND already0 IS NULL AND event0_exists IS NULL THEN
    INSERT INTO demo.events (event_id, event_time, user_id, event_type, payload)
    VALUES (${EVENT0}, toTimestamp(now()), '${USER_ID}', '${EVENT_TYPE}', 'txn_demo_seq0');

    INSERT INTO demo.session_timeline (user_id, session_id, seq, event_id, event_time, event_type, payload)
    VALUES ('${USER_ID}', ${SESSION_ID}, 0, ${EVENT0}, toTimestamp(now()), '${EVENT_TYPE}', 'txn_demo_seq0');

    INSERT INTO demo.session_seq_applied (user_id, session_id, seq)
    VALUES ('${USER_ID}', ${SESSION_ID}, 0);
  END IF
 COMMIT TRANSACTION;

 SELECT * FROM demo.session_timeline;

-- Next Accord Transaction on seq=1
--  (Apply seq=1 ONLY IF seq=0 is already applied (in-order gating))
 BEGIN TRANSACTION
  LET session_ok = (SELECT session_id
                    FROM demo.sessions_open
                    WHERE user_id='${USER_ID}' AND session_id=${SESSION_ID});

  LET already1 = (SELECT seq
                  FROM demo.session_seq_applied
                  WHERE user_id='${USER_ID}' AND session_id=${SESSION_ID} AND seq=1);

  LET prev_ok = (SELECT seq
                 FROM demo.session_seq_applied
                 WHERE user_id='${USER_ID}' AND session_id=${SESSION_ID} AND seq=0);

  LET event1_exists = (SELECT event_id
                       FROM demo.events
                       WHERE event_id=${EVENT1});

  IF session_ok IS NOT NULL AND prev_ok IS NOT NULL AND already1 IS NULL AND event1_exists IS NULL THEN
    INSERT INTO demo.events (event_id, event_time, user_id, event_type, payload)
    VALUES (${EVENT1}, toTimestamp(now()), '${USER_ID}', '${EVENT_TYPE}', 'txn_demo_seq1');

    INSERT INTO demo.session_timeline (user_id, session_id, seq, event_id, event_time, event_type, payload)
    VALUES ('${USER_ID}', ${SESSION_ID}, 1, ${EVENT1}, toTimestamp(now()), '${EVENT_TYPE}', 'txn_demo_seq1');
...
Simple query demonstrating Jeff's prototype for Postgres dialect SQL directly against Cassandra.
```
todo
```
...
todo
```
• Move terabytes of Parquet files quickly into the database
```
todo
```
