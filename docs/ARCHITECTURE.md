...
- A claim of full feature parity with mature, general-purpose SQL databases.
- A promise that every enterprise workload can be consolidated immediately without trade-offs.
- A turnkey drop-in replacement for warehouses and lakes in every scenario — some organizations will still choose to export to Parquet/Iceberg for cost/performance/lifecycle reasons, and that's legitimate.
